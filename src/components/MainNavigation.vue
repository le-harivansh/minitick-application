...
const authenticatedUser = useMainStore().authenticatedUser;

const isAuthenticated = computed(
  () => !!authenticatedUser.id && !!authenticatedUser.username
);
</script>

<template>
  <nav class="py-2 border-b-2 border-slate">
    <section v-if="isAuthenticated" class="flex">
      <p class="flex-1 font-semibold tracking-wide text-lg">
        {{ authenticatedUser.username }}
      </p>
      <RouterLink
        :to="{ name: 'home' }"
        class="focus:outline-none text-slate-700"
